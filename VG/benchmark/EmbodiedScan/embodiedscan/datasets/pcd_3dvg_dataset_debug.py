...
class PointCloud3DGroundingDatasetDEBUG(BaseDataset):
    ...
    def __init__(self,
        ...
        vg_file = 'es_gen_text/vg_full/VG_val_10Percent_flattened_token_positive.json'
    ...
    def convert_info_to_scan(self):
        ...
        for data in self.data_list:
            scan_id = data['sample_idx']
            data.update({'scan_id':scan_id})
            self.scans[scan_id] = data
        print("number of true scans:", len(list(self.scans.keys())))
    ...
    def load_language_data(self):
        ...
        for anno in mmengine.track_iter_progress(language_annotations):
            ...
            if not new_text.startswith('thereisagreencolor,smoothtexture,plasticmaterialbottlewithacylindricalshape.findalltheitemssmallerthanthebottleintheroom'):
                continue #HACK: for debugging only
            language_info = dict()
            ...
            if not language_info['scan_id'] in self.scans:
                num_dropped_by_not_scan_id += 1
                continue #HACK: only happens at debugging mini data
