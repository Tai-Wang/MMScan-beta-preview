...
class ProposalModule(nn.Module):
    def __init__(self, num_class,num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256):
        ...
        self.proposal = nn.Sequential(
            ...
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Conv1d(128,2+3+self.angle_bin+num_size_cluster*4+self.num_class,1)
        )
    ...
    def forward(self, xyz, features, data_dict):
        ...
        data_dict = self.decode_scores(net, data_dict, self.num_class, self.num_size_cluster, self.mean_size_arr)

        return data_dict

    def decode_scores(self, net, data_dict, num_class, num_size_cluster, mean_size_arr):
        """
        ...
        size_scores = net_transposed[:,:,5+self.angle_bin : 5+self.angle_bin+num_size_cluster]
        size_residuals_normalized = net_transposed[:,:,5+self.angle_bin+num_size_cluster : 5+self.angle_bin+num_size_cluster*4].view([batch_size, num_proposal, num_size_cluster, 3]) # Bxnum_proposalxnum_size_clusterx3
        
        sem_cls_scores = net_transposed[:,:,5+self.angle_bin+num_size_cluster*4:] # Bxnum_proposalx10
