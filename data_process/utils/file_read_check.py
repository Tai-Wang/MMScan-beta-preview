...
def check_dict_matching(dict1,dict2):
...
def refine_text(text):
    ...
    text = "? ".join(sentences)
    if text[-1] not in [".", "?"] and text[-2:] not in [". ", "? "]:
        text += ". "

    up_text = ''
    for i in range(len(text)):
        if (text[i] in ['i','x']) and ((i==0 or not text[i-1].isalpha()) and (i==len(text)-1 or not text[i+1].isalpha())):
            up_text += text[i].upper()
