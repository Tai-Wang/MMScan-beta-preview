...
class Dataset(ScanNetBaseDataset):
    ...
    def __init__(
        ...
        anno_files = glob(osp.join(embodied_scan_data_base,f"QA/*.json"))
        to_skip = []
        for l in anno_files:
            extrac = l.split("/")[-1].split(".")[0].split("_")
            extrac = "_".join(extrac[:-1])
            if extrac not in self.scan_names:
                to_skip.append(l)
        for l in to_skip:
            anno_files.remove(l)
        self.embodied_scan_anno = []
        for p in anno_files:
            self.embodied_scan_anno+=json.load(open(p))
        ...
        #    }]
        self.annotations = self.embodied_scan_anno
        # if self.split != 'train':
        #     self.annotations = [{'scene_id': scene_id} for scene_id in self.scan_names]
        self._tag_dataset(self.annotations, 'embodied_qa')
    ...
    def __getitem__(self, idx):
        ...
        embodied_scan_bbox = []
        if self.annotations[idx]["input_bboxes"] is not None:
            for bbox in self.annotations[idx]["input_bboxes"]:
                embodied_scan_bbox.append(torch.tensor(bbox))
        # if self.annotations[idx]['output_bboxes_id'] is not None:
        #     for bbox in self.annotations[idx]['output_bboxes_id']:
        #         embodied_scan_bbox.append(torch.tensor(self.embodied_scan[scan_name][bbox]['bbox_3d']))
        ...
        boxes = None
        if self.annotations[idx]['input_bboxes_id'] is not None:
            boxes = self._encode_box_coords(ret_dict,len(self.annotations[idx]['input_bboxes_id']))
        ...
        if self.split == 'train':
            caption = self.annotations[idx]['answers'][0] #' '.join(self.annotations[idx]['token'])
        else:
            caption = ""
        # TODO:flatten the answer
        
        ## reference object

        
        # from all bbox get specific instance mask
        # TODO: need to check the bbox coordinate
        # boxes = None
        # if self.annotations[idx]['output_bboxes_id'] is not None:
        #     boxes = self._encode_box_coords(ret_dict,len(self.annotations[idx]['output_bboxes_id']))
