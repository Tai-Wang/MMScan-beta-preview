...
from .parse_scene_name import map3rscan2origin
...
class ScanNetBaseDataset(Dataset):
    def __init__(
        ...
        if split_set == "all":
            ...
            """
            We load embodiedscan scenes under the child class
            """
    ...
    def _get_scan_data(self, scan_name,embodied_scan_bbox=None):
        pcd_data = torch.load(os.path.join("../../data/mmscan_scenes",
                                             f'{map3rscan2origin(scan_name)}.pth'))
        mesh_vertices = np.concatenate((pcd_data[0],pcd_data[1]),axis=1)
        instance_labels = pcd_data[-1]
        semantic_labels = pcd_data[-2]
        MAX_NUM_OBJ = self.dataset_config.max_num_obj
        
        instance_bboxes = [np.array(self.embodied_scan[scan_name][ii]['bbox_3d'][:6]+[0]+[self.embodied_scan[scan_name][ii]['bbox_id']]) for ii in self.embodied_scan[scan_name]]
        ...
        if self.use_normal:
            normals = np.zeros(mesh_vertices[:,0:3].shape) #mesh_vertices[:,6:9].shape
        ...
        # ------------------------------- DATA AUGMENTATION ------------------------------
        if embodied_scan_bbox is not None:
            embodied_scan_bbox = embodied_scan_bbox.numpy()
            if len(embodied_scan_bbox.shape)==1:
                embodied_scan_bbox = np.expand_dims(embodied_scan_bbox, axis=0)
        if self.augment:
            ...
            if np.random.random() > 0.5:
                ...
                if embodied_scan_bbox is not None:
                    embodied_scan_bbox[:, 0] = -1 * embodied_scan_bbox[:, 0]
            ...
            if np.random.random() > 0.5:
                ...
                if embodied_scan_bbox is not None:
                    embodied_scan_bbox[:, 1] = -1 * embodied_scan_bbox[:, 1]
            ...
            if embodied_scan_bbox is not None:
                embodied_scan_bbox = self.dataset_config.rotate_aligned_boxes(
                embodied_scan_bbox, rot_mat
            )

        raw_sizes = target_bboxes[:, 3:6]
        if embodied_scan_bbox is not None:
            embodied_scan_raw_sizes = embodied_scan_bbox[:,3:6]
        ...
        if embodied_scan_bbox is not None:
            embodied_scan_box_centers = embodied_scan_bbox.astype(np.float32)[:, 0:3]
            ...
            embodied_scan_box_centers_normalized = embodied_scan_box_centers_normalized.squeeze(0)
        ...
        if embodied_scan_bbox is not None:
            ...
            embodied_scan_box_sizes_normalized = embodied_scan_box_sizes_normalized.squeeze(0)
        ...
        if embodied_scan_bbox is not None:
            embodied_scan_box_corners = self.dataset_config.box_parametrization_to_corners_np(
                embodied_scan_box_centers[None, ...],
                embodied_scan_raw_sizes.astype(np.float32)[None, ...],
                np.zeros((embodied_scan_box_centers.shape[0],), dtype=np.float32).astype(np.float32)[None, ...],
            )
        ...
        ret_dict = {}
        if embodied_scan_bbox is not None:
            ret_dict["embodied_scan_box_corners"] = embodied_scan_box_corners.astype(np.float32)
            ret_dict["embodied_scan_box_centers"] = embodied_scan_box_centers.astype(np.float32)
            ret_dict["embodied_scan_box_centers_normalized"] =embodied_scan_box_centers_normalized.astype(
                np.float32
            )
            ret_dict["embodied_scan_box_sizes_normalized"] = embodied_scan_box_sizes_normalized.astype(np.float32)
        else:
            ret_dict["embodied_scan_box_corners"] = np.zeros((MAX_NUM_OBJ,24)).astype(np.float32)
            ret_dict["embodied_scan_box_centers"] = np.zeros((MAX_NUM_OBJ,3)).astype(np.float32)
            ret_dict["embodied_scan_box_centers_normalized"] =np.zeros((MAX_NUM_OBJ,3)).astype(
                np.float32
            )
            ret_dict["embodied_scan_box_sizes_normalized"] = np.zeros((MAX_NUM_OBJ,3)).astype(np.float32)
