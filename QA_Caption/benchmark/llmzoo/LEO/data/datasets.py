...
class LeoBase(Dataset):
    ...
    def load_scannet(self, scan_id):
        scan = {}
        pcd_data = torch.load(os.path.join("../../data/mmscan_scenes", f'{scan_id}.pth'))
...
from .parse_scene_name import map3rscan2origin,parse_embodied_scan
...
class LeoEmbodiedScanL(LeoBase):
    ...
    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.embodied_scan_l.anno_dir
        ...
        if split == 'train':
            self.split = 'train'
            self.pc_type = 'gt'
            anno_file = os.path.join(self.embodied_scan_info_base,"embodiedscan_infos_train_full.pkl")
        else:
            self.split = 'val'
            self.pc_type = getattr(cfg.data.embodied_scan_c, 'pc_type', 'gt')
            anno_file = os.path.join(self.embodied_scan_info_base,"embodiedscan_infos_val_full.pkl")

        logger.info(f"Loading embodied_scan_QA {split}-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.embodied_scan_l.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading embodied_scan_QA {split}-set language, collected {len(self.scan_ids)} data")
        self.embodied_scan = []
        
        with open(anno_file,'rb') as f:
            self.embodied_scan = pickle.load(f)['data_list']
        collect_dict = {}
    ...
    def load_anno(self, anno_dir):
        ...
        scan_caps = []
        if self.split != 'train':
            scenes_to_load = os.path.join(self.cfg.data.embodied_scan_anno_base,"test_scene_ids.txt")
        else:
            scenes_to_load = os.path.join(self.cfg.data.embodied_scan_anno_base,"train_scene_ids.txt")
        with open(scenes_to_load, 'r') as file:
        ...
        print( f" ============ loading all QA under {anno_dir} ============ ")
        for l in anno_files:
            extrac = l.split("/")[-1].split(".")[0].split("_")
            extrac = "_".join(extrac[:-1])
            if extrac not in regions:
                to_skip.append(l)
        for l in to_skip:
            anno_files.remove(l)
        ...
        for anno_file in anno_files:
            with open(anno_file, 'r') as f:
                json_data = json.load(f)
            for item in json_data:
                obj_id = item['input_bboxes_id'] # is a list
                cap = item['answers']
                # add all answers
                if self.split == 'train': 
                    for c in cap:
    ...
    def __getitem__(self, index):
        scan_id = self.scan_ids[index]
        lang_meta = self.lang_data[index]
        obj_id = lang_meta['obj_id']    # embodied scan bbox ids
        obj_caption = lang_meta['caption']

        # load pcds
        if scan_id not in self.scan_data:
            self.scan_data[scan_id] = self.load_scannet(map3rscan2origin(scan_id)) #3rscan --> unreadable format
        obj_pcds = self.scan_data[scan_id]['obj_pcds'].copy()   # Dict{int: np.ndarray (N, 6)}
        iou_flag = 1
        if obj_id is not None and self.scan_data[scan_id]['pcds'].size:
            all_obj_mask = []
            try:
                for i_obj_id in obj_id:
                    bbox = self.embodied_scan[scan_id][i_obj_id]['bbox_3d']
                    orientation=R.from_euler("zxy",bbox[6:],degrees = False).as_matrix().tolist()
                    position=np.array(bbox[:3])
                    size=np.array(bbox[3:6])
                    all_obj_mask.append(torch.tensor(is_inside_box(self.scan_data[scan_id]['pcds'][:,:3],position,size,orientation),dtype=bool))
                ...
                if query_instance_mask.numel() >0:
                    selected_obj_pcds = [ self.scan_data[scan_id]['pcds'][query_instance_mask] ]    # here use embodied scan bbox to crop pcd
                else:
        ...
        data_dict = self.get_prompts(
            ...
            dialogue=lang_meta['question']
        )
        data_dict.update({
            'source': 'scannet',
            'scene_id': scan_id,
            'question_id':lang_meta['ID'],
            'obj_fts': obj_fts,
            'obj_locs': obj_locs,
            'anchor_locs': anchor_loc,
            'img_fts': torch.zeros(3, 224, 224),
...
@DATASET_REGISTRY.register()
class LeoEmbodiedScanC(LeoBase):
    situation_pool = Leo_situation_pool
    instruction_pool = Leo_objcap_instruction_pool

    def __init__(self, cfg, split):
        super().__init__()
        self.scannet_base = cfg.data.embodied_scan_c.anno_dir
        self.num_points = cfg.data.embodied_scan_c.num_points
        self.max_obj_len = cfg.data.embodied_scan_c.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)
        self.embodied_scan_info_base = cfg.data.embodied_scan_info_base
        self.cfg = cfg
        if split == 'train':
        ...
        logger.info(f"Loading embodied_scan_caption {split}-set language")
        self.scan_ids, self.lang_data = self.load_anno(cfg.data.embodied_scan_c.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading embodied_scan_caption {split}-set language, collected {len(self.scan_ids)} data")
        self.embodied_scan = []
    ...
    def load_anno(self, anno_dir):
        ...
        with open(scenes_to_load, 'r') as file:
            regions = file.readlines()
        regions =set( [region.strip() for region in regions] )

        for anno_file in anno_dir:
            with open(anno_file, 'r') as f:
                json_data = json.load(f)
            for item in json_data:
                if item['scan_id'] not in regions:
                    continue
        ...
        print( f'=================== load {len(scan_caps)} caption for train')
        return scan_ids, scan_caps
...
class LeoMix(Dataset):
    mapping = {
        ...
        'embodied_scan_c':LeoEmbodiedScanC,
    }
