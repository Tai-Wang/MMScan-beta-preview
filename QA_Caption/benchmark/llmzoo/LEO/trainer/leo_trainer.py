        ...
        for task_name in self.evaluators.keys():
            if task_name in self.data_loaders['test']:
                ...
                for idx,data_dict in enumerate(loader):
                    ...
                    self.evaluators[task_name].update(data_dict)
                    pbar.update(1)
                    print(self.evaluators[task_name].save_results[-1])
                    if idx % 500 == 0:
                        json.dump(self.evaluators[task_name].save_results,open(os.path.join(self.exp_dir,f"test_{task_name}_{idx}.json"),'w'))
                json.dump(self.evaluators[task_name].save_results,open( os.path.join(self.exp_dir,f"test_{task_name}_complete.json"),'w'))
