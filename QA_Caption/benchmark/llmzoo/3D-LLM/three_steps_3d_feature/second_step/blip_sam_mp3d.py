...
if __name__ == "__main__":
    ...
    data_list = sorted(os.listdir(args.scene_dir_path))

    test_set = pickle.load(open("embodiedscan_infos/embodiedscan_infos_test.pkl",'rb'))
    test_set = test_set['data_list']
    all_scene = [i['sample_idx'] for i in test_set]
    trscene = []
    for s in all_scene:
        if "matterport3d" in s:
            if s.split("/")[-2] not in trscene:
                trscene.append(s.split("/")[-2])
    data_list = trscene
    ...
    for scene in tqdm(data_list):
        try:
            ...
            for file in os.listdir(os.path.join(args.mask_dir_path, scene)):
                INPUT_IMAGE_PATH = os.path.join(args.scene_dir_path, scene,"matterport_color_images", file.replace(".pt", ".jpg"))
                ...
                print(outfeat.shape)
                outfeat = outfeat.numpy()
                np.savez_compressed(
                    SEMIGLOBAL_FEAT_SAVE_FILE,
                    feat = outfeat
                )
                print(f"delete {MASK_LOAD_FILE}")
                os.remove(MASK_LOAD_FILE)
